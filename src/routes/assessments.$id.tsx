  ...
  const query = useQuery<FullPaper | null>({
    ...
    queryFn: async () => {
      ...
      const { data: exs, error: exErr } = await supabase
        .from("exercises")
        .select("id,number,kind,rubric,intro,statements,questions(id,number,stem,correct_letter,speaker_index,question_options(letter,text,image_prompt)),listening_scripts(sequence,speaker_label,transcript)")
        .eq("assessment_id", id)
        .order("number");
